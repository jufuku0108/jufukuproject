...
class ListHttpLogs(LoginRequiredMixin, View):
    
    def get(self, request, *args, **kwargs):
        try:
            oauth2_session = CustomizeOauthSession.get_oauth2_session(self.request)
            url = f'{settings.API_BASEURL}/system/httplogs'
            ...
            context = {
                'httploglist': response.json(),
                'pagination': json.loads(response.headers.get('X-Pagination'))
            }        

            return TemplateResponse(request, 'logs/list_httplogs.html', context)
